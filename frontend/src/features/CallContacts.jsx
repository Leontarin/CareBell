...
export default function CallContacts() {
  ...
  useEffect(() => {
    // If we don't have a user yet, show empty state (no error)
    if (!userId) {
      setContacts([]);
      setError(null);
      setLoading(false);
    ...
    axios
      .get(`${API}/contacts/getAll/${encodeURIComponent(userId)}`)
      .then((res) => {
        if (cancelled) return;
  ...
  const visibleContacts = useMemo(() => {
    ...
    return contacts
      .filter((c) => {
        ...
        const phone = (c.phoneNumber || "").toString();
        const rel = (c.relationship || "").toLowerCase();
        return fullName.includes(q) || phone.includes(q) || rel.includes(q);
      })
      .sort((a, b) =>
  ...
  const handleBulkDelete = () => {
    ...
    setMenuOpen(false);
    if (!window.confirm(t("CallContacts.confirmBulkDelete", { count: selectedIds.size }))) return;
  ...
  if (loading) return <p className="text-center py-8">{t("CallContacts.loading")}</p>;
  if (error)
    return (
            ...
            <ul className="py-1">
              <li>
                <button
                  ...
                  className="text-left px-4 py-2 text-sm text-gray-900 dark:text-gray-100 font-semibold hover:bg-red-300 dark:hover:bg-red-600 disabled:opacity-50"
                >

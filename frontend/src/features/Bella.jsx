...
export default function Bella() {
  ...
  useEffect(() => {
    ...
    vapi.on('call-start', async () => {
      ...
      // Get a human-readable date string (e.g., "Saturday, February 7, 2026, 11:15 AM")
      const now_time = new Date();
      const dateString = now_time.toLocaleDateString('en-US', { 
        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' 
      });
      const timeString = now_time.toLocaleTimeString('en-US', { 
        hour: '2-digit', minute: '2-digit' 
      });

      console.log(`Injecting Time: ${dateString} at ${timeString}`);

      await vapi.send({
        type: 'add-message',
        message: {
          role: 'system',
          content: `Current System Time: ${dateString}, ${timeString}. 
                    All relative time references (like 'tomorrow' or 'yesterday') refer to this moment.`
        }
        });
...
//       } catch (e) { console.error(e); }
      ...
      try {
        ...
        if (Array.isArray(rems) && rems.length > 0) {
          // UPDATE: Map the new 'category' and 'content' fields
          const memoryStrings = rems.map(r => {
            // Use content if available (new format), fallback to entities (old format)
            if (r.content) {
              return `[${r.category}] ${r.content}`;
            }
            // Fallback for old data or strict entity-only data
            const ents = r.entities || {};
            const entStr = Object.entries(ents)
              .map(([k,v]) => `${k.replace(/_/g,' ')}: ${v}`)
              .join(', ');
            return `[${r.category}] ${entStr}`;
          });

          const message = `Remember the following facts about the user from past conversations: ${memoryStrings.join('; ')}`;
